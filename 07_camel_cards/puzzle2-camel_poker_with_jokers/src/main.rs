...
impl Hand {
    ...
    fn determine_rank(cards: &Vec<Card>) -> HandRank {
        ...
        // get jokers and remove from counts
        let n_jokers = *counts.entry(Card::Joker).or_insert(0);
        ...
        if n_two == 2 && n_jokers == 1 {
            return HandRank::FullHouse;
        }
        if has_three {
            return HandRank::ThreeOfAKind;
        }
        if n_two == 1 && n_jokers == 1 {
            return HandRank::ThreeOfAKind;
        }
        if n_two == 2 {
