...
fn main() {
    ...
    // Get sum of power of cubes
    let sum_of_powers = input_data.lines()
        .map(|s| parse_game(s).power_of_cubes())
        .sum::<u32>();

    println!("Sum of possible IDs: {}", sum_of_ids);
    println!("Sum of power of cubes: {}", sum_of_powers);
}
...
impl Game {
    ...
    fn maximum_value_each_color(&self) -> GameResult {
        // return the maximum number of cubes for each color in each result
        // this is the minimum number of cubes in the bag
        let mut n_blue: u32 = 0;
        let mut n_red: u32 = 0;
        let mut n_green: u32 = 0;

        for result in &self.game_result {
            if result.n_blue > n_blue {
                n_blue = result.n_blue;
            }
            if result.n_red > n_red {
                n_red = result.n_red;
            }
            if result.n_green > n_green {
    ...
    fn power_of_cubes(&self) -> u32 {
        // return the sum of the cubes for the maximum of all games
        let max_values = self.maximum_value_each_color();

        max_values.n_red * max_values.n_green * max_values.n_blue
    }
}
